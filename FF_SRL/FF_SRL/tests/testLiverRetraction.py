...
class TestStandalone():
    def __init__(self, numEnvs=1, numAct=3, debugTime=True, useGraph=True, visualize=True) -> None:
        ...
        self.stage = Usd.Stage.Open("../scenes/liverRetraction.usd")
        ...
        self.simBVH = dk.SimBVH(self.device, self.simModel, binsNumber=8, load=True, save=False, path=("../scenes/liverRetraction.npz"))

        self.renderer = dk.render.WarpRaycastRendererDO(device=self.device, simModel=self.simModel, resolution=256, cameraPos=[0.0, 50.0, 50.0], cameraRot=[0.0, 0.0, 0.0], lightPos=[0.0, 35.0, 50.0], lightIntensity=0.03, mode="gpu")
        self.renderer.lookAt([0.0, 10, 0.0])

        self.simIntegrator = dk.SimIntegratorDO(self.device)

        self.remoteCenterOfMotionPos = torch.tensor([9.036183, 35.260103, 1.567807], dtype = torch.float, device = self.device).repeat(self.numEnvs, 1)
        self.remoteCenterOfMotionRot = torch.tensor([0.0, 0.0, 1.0], dtype = torch.float, device = self.device).repeat(self.numEnvs, 1)
...
testStandalone = TestStandalone(numEnvs=nEnvs, debugTime=False, useGraph=False, visualize=True)
